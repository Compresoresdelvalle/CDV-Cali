...
- **F1-08 (P2, datos):** 1 fila de `inventario` con `estado_stock` stale — FA-2236 en ALM-01: marcada `'Agotado'` con `cantidad=1` (debería ser `'Bajo'`). Fila anterior a un trigger funcional. Requiere autorización del usuario para el `UPDATE` correctivo en producción.
