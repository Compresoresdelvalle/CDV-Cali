...
export default function ProductoDetalle() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12">
      {/* ── Header ───────────────────────────────────────────────────── */}
      <div
        className="sticky top-0 z-10 bg-white border-b border-border px-4 py-3
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-surface/40">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  ...
                  {producto.marca && (
                    <Tag variant="accent">{producto.marca}</Tag>
                  )}
            ...
            {producto.descripcion && (
              <p className="text-xs text-text-sub mt-3 leading-relaxed">
                {producto.descripcion}
              </p>
            )}
          </div>
          ...
          {/* Precios */}
          <div
            className={`grid divide-x divide-border ${esAdmin ? "grid-cols-2" : "grid-cols-1"}`}
          >
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-border">
                  <th className="px-4 py-2.5 text-xs font-semibold text-text-sub">
                    Sede
                  </th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-text-sub text-right">
                    Cant.
                  </th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-text-sub">
                    Estado
                  </th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-text-sub hidden sm:table-cell">
                    Ubicación
                  </th>
                </tr>
              </thead>
                ...
                <li key={mov.id} className="px-4 py-3 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`text-sm font-bold tabular-nums ${mov.cantidad >= 0 ? "text-stock-ok" : "text-stock-out"}`}
                      >
                        {mov.cantidad >= 0 ? "+" : ""}
                        {mov.cantidad}
                      </span>
                    ...
                    {mov.observaciones && (
                      <p className="text-[11px] text-text-sub mt-1 italic truncate">
                        {mov.observaciones}
                      </p>
                    )}
