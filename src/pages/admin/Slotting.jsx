...
export default function Slotting() {
  ...
  const esSubida = (i) => i.prioridad_sugerida < i.prioridad_actual;

  const aplicar = async (item) => {
    const ok = await confirm({
      titulo: "Aplicar sugerencia de ubicación",
      mensaje: `Se moverá "${item.nombre}" de ${item.ubicacion_actual} a ${item.ubicacion_sugerida} en ${sedeLabel(item.sede_id)}.`,
    ...
    >
      <div>
        ...
        >
          ...
          que más se mueve y sacar de las zonas premium lo que no rota.
        </p>
      </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={pillStyle(
                          esSubida(i) ? "--warning" : "--muted-foreground",
                        )}
                      >
                        {i.motivo}
                      </span>
          ...
          <ul className="md:hidden divide-y" role="list">
            {filtrados.map((i) => (
              ...
              >
                ...
                <span
                  className="mb-3 inline-flex max-w-full items-center rounded-[4px] border px-1.5 py-0.5 text-[11px] font-medium leading-snug"
                  style={pillStyle(
                    esSubida(i) ? "--warning" : "--muted-foreground",
                  )}
                >
