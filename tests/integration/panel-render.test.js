...
describe("Composicion — poco margen Y volumen", () => {
  // El caso que motiva todo esto: una venta suelta con margen pesimo compite
  ...
  it("ofrece filtrar por peso en la venta", async () => {
    const html = await montar();
    expect(html).toContain("Peso en la venta");
    for (const u of ["Todos", "≥ 1%", "≥ 3%", "≥ 5%"]) {
      expect(html).toContain(u);
    }
  });

  it("con 'ver los peores' deja elegir entre porcentaje y plata", async () => {
    const html = await montar({ peores: true });
    expect(html).toContain("Peor según");
    expect(html).toContain("% de margen");
    expect(html).toContain("plata que deja");
  });

  it("sin filtro el pie dice que cuadra con la cascada", async () => {
    expect(await montar()).toContain(
