...
function textoDelPDF() {
  const cotizacion = {
    numero: "TEST-1", fecha: "2026-08-20T12:00:00Z", sede_id: "CV",
    cliente_nombre: "Cliente de prueba", cliente_nit: "900123456",
    subtotal: 100000, descuento_valor: 0, iva_pct: 19, domicilio: 0,
    total: 119000, vigencia_dias: 15, observaciones: null,
  };
  const items = [{ producto: { referencia: "FA-2236", nombre: "Filtro" }, cantidad: 2, precio_unitario: 50000, subtotal: 100000 }];
  // titular null a propósito: debe caer al nombre LEGAL de la empresa
  const cuentas = [{ banco: "Bancolombia", tipo: "Ahorros", numero: "123-456", titular: null }];

  const pdf = generarCotizacionPDF({ cotizacion, items, cuentas, vendedor: "Maritza" });
  ...
  for (;;) {
    ...
    try {
      texto += zlib.inflateSync(buf.subarray(desde, fin)).toString("latin1") + "\n";
    } catch {
