 ...
 *
 * Cubre:
 *   ✓ Devolución de cliente ('cliente') — suma stock en sede
 *   ✓ Devolución de proveedor ('proveedor') — suma stock en sede
...
describe("registrar-devolucion", () => {
  ...
  it("tipo=cliente — suma 2 unidades al stock", async () => {
    ...
    const { status, data } = await invokeAs(FN, "pedro", {
      ...
      cantidad: 2,
      motivo: "TEST: producto en buen estado, cliente cambió de parecer",
    });
  ...
  it("tipo=cliente — crea movimiento de auditoría tipo devolucion", async () => {
    ...
    expect(movs[0].cantidad).toBeGreaterThan(0);
  });
  ...
  it("tipo=proveedor — suma 3 unidades al stock", async () => {
    const antes = await getProducto(adminClient, "FA-2236");
    ...
    const { status, data } = await invokeAs(FN, "pedro", {
      ...
      sede_id: "BOD-PRINCIPAL",
      cantidad: 3,
      motivo: "TEST: proveedor envió de más",
    });
