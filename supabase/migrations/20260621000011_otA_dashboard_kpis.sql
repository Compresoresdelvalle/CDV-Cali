-- ============================================================================
-- Rediseño OT — Opción A — FASE 5c: Dashboard sin doble conteo.
-- Las tarjetas de "ventas" cuentan solo ventas DIRECTAS (origen='directa').
-- "ingresos_servicios_mes" = ventas-OT + abonos aún no conciliados (igual al
-- cierre). "ordenes_abiertas" = toda OT no entregada/cancelada (flujo nuevo).
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'ticket_promedio_mes', (select coalesce(avg(total),0)::integer from ventas where (fecha at time zone 'America/Bogota')::date >= v_mes_inicio and anulada = false and origen = 'directa' and (v_rol = 'Admin' or sede_id = v_sede)),
    ...
    'ingresos_servicios_mes', (
      (select coalesce(sum(total),0) from ventas where origen = 'ot' and anulada = false and (fecha at time zone 'America/Bogota')::date >= v_mes_inicio and (v_rol = 'Admin' or sede_id = v_sede))
      + (select coalesce(sum(a.monto),0) from abonos a join ordenes_servicio o on o.id = a.orden_id where a.venta_id is null and o.estado <> 'cancelada' and (a.fecha at time zone 'America/Bogota')::date >= v_mes_inicio and (v_rol = 'Admin' or o.sede_id = v_sede))
    ),
