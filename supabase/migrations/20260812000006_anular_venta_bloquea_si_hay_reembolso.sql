-- fn_anular_venta: se agrega un guard para NO anular una venta que ya tiene un
-- reembolso registrado (devolución con reembolso o garantía "devolver dinero"):
-- la plata ya salió de la caja, y anular volvería a descontar el ingreso =
-- doble descuento. Todo lo demás queda idéntico.
...
begin
  ...
  -- Guard anti doble-descuento.
  if exists (select 1 from devoluciones where venta_id = p_venta_id and estado <> 'anulada' and coalesce(monto_reembolso,0) > 0)
     or exists (select 1 from garantias_venta where venta_id = p_venta_id and resolucion = 'devolver_dinero' and estado <> 'anulada') then
    raise exception 'Esta venta ya tiene un reembolso registrado; anularla descontaría la plata dos veces. Anula primero el reembolso (la devolución o la garantía) si necesitas revertir.';
  end if;
