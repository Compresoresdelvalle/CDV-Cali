-- H4: tres precisiones de dinero en fn_dashboard_kpis. No se elimina ningun campo del contrato.
--  (a) ticket_promedio_mes incluia ventas con total = 0 (20 este mes), bajando el promedio de
--      $278.609 a $263.549. Ahora se excluyen del promedio; siguen contando en ventas_mes.
--  (b) devoluciones_mes valoriza con respaldo a productos.precio_venta (catalogo) cuando la
--      devolucion no tiene venta_id, lo que puede sobre/subestimar. No hay mejor valorizacion
--      disponible (sin venta_id no existe precio de transaccion en ninguna tabla), asi que se
--      deja igual pero se expone devoluciones_mes_sin_venta para que la parte estimada sea
--      auditable.
--  (c) los anticipos de OT aun no facturadas dejaron de aparecer en todo KPI al quitar
--      ingresos_servicios_mes del calculo (para evitar el doble conteo). Es plata real que
--      entro. Se agrega anticipos_ot_sin_facturar como indicador propio: NO se suma a ventas
--      ni al margen.
...
begin
  ...
  select jsonb_build_object(
    'ventas_hoy', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date = v_hoy and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'ventas_ayer', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date = v_hoy - 1 and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'ventas_semana', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date >= v_semana_inicio and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'ventas_semana_prev', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date >= v_semana_inicio - 7 and (fecha at time zone 'America/Bogota')::date < v_semana_inicio and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'ventas_mes', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date >= v_mes_inicio and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'ventas_mes_prev', (select coalesce(sum(total),0) from ventas where (fecha at time zone 'America/Bogota')::date >= (v_mes_inicio - interval '1 month')::date and (fecha at time zone 'America/Bogota')::date < v_mes_inicio and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    -- H4(a): las ventas con total = 0 (cortesias, garantias, reposiciones) siguen contando en
    -- ventas_mes pero se excluyen del ticket promedio para que no lo distorsionen a la baja.
    'ticket_promedio_mes', (select coalesce(avg(total),0)::integer from ventas where (fecha at time zone 'America/Bogota')::date >= v_mes_inicio and anulada = false and origen in ('directa','ot') and total > 0 and (v_rol = 'Admin' or sede_id = v_sede)),
    'devoluciones_mes', (
    ...
    -- H4(b): las devoluciones sin venta_id no tienen precio de transaccion en ninguna parte, asi
    -- que se valorizan con productos.precio_venta (catalogo) y pueden sobre/subestimar. Se expone
    -- aparte cuanto de devoluciones_mes es esa parte estimada, para que la cifra sea auditable.
    ...
    -- H4(c): plata real que entro por abonos de OT que todavia NO tienen venta asociada. No se
    -- suma a ventas ni al margen (eso reintroduciria el doble conteo); es un indicador propio
    -- para que el dinero no quede invisible en el Dashboard.
