-- ============================================================================
-- Bloque 3 — #13 (cuenta bancaria) + salvaguarda F4-05 (log de precio editado).
--
-- #13: registrar a qué CUENTA BANCARIA entró el pago de la venta.
--   - `ventas.cuenta_bancaria TEXT` (nullable; opcional, sobre todo para
--     Transferencia/Tarjeta).
--   - `fn_registrar_venta` recibe `p_cuenta_bancaria` y lo guarda.
--
-- F4-05 (salvaguarda del #9): el #9 dejó el precio editable por el operario
--   (antes era server-authoritative). Para auditar, guardamos el precio de
--   CATÁLOGO vigente al momento de la venta junto al precio efectivamente
--   cobrado. "Precio editado" = (precio_unitario <> precio_catalogo).
--   - `detalle_venta.precio_catalogo NUMERIC` (nullable; histórico).
--
-- Se conserva todo lo del #9 (precio por ítem + p_iva_pct) y de Bloque 1
-- (gate de rol + bloqueo de venta por sede). Cambio de firma → DROP+CREATE;
-- los dos parámetros nuevos tienen DEFAULT, así que las llamadas viejas siguen.
-- ============================================================================

ALTER TABLE public.ventas        ADD COLUMN IF NOT EXISTS cuenta_bancaria TEXT;
ALTER TABLE public.detalle_venta ADD COLUMN IF NOT EXISTS precio_catalogo NUMERIC;

DROP FUNCTION IF EXISTS public.fn_registrar_venta(text, text, text, text, numeric, text, jsonb, numeric);
...
BEGIN
  ...
  INSERT INTO ventas (
    ...
    NULLIF(btrim(COALESCE(p_cuenta_bancaria, '')), '')  -- #13
  )
  ...
  LOOP
    ...
    -- F4-05: guardamos también el precio de catálogo para auditar ediciones.
