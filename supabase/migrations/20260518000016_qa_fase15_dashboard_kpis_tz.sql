-- QA Campaña — Fase 15 (Dashboard + Cierres)
-- F15-01 (P2, correctitud): `fn_dashboard_kpis` agrupaba las ventas por día
-- UTC (`fecha::date = CURRENT_DATE`), mientras `_fn_cierre_totales` usa el día
-- de `America/Bogota`. Una venta hecha de noche en Colombia caía en el día UTC
-- siguiente → el dashboard "ventas hoy" no cuadraba con el cierre del mismo
-- día. CLAUDE.md exige zona `America/Bogota`. Se recalcula todo el bucketing
-- de fechas usando el día de Bogota, consistente con los cierres.
-- (Función de solo lectura — sin riesgo de integridad.)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'compras_mes', (SELECT COALESCE(SUM(total),0) FROM compras WHERE (fecha AT TIME ZONE 'America/Bogota')::date >= v_mes_inicio AND (v_rol = 'Admin' OR sede_destino_id = v_sede)),
