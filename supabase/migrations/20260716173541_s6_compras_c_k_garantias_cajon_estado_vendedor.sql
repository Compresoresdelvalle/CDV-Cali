-- S6-C: garantía descuenta/devuelve del cajón correcto (venta vs insumo)
-- S6-K: bloquear garantía sobre compra cancelada
-- Decisión dueño: Vendedor puede abrir garantías; estado devolucion_garantia conectado

ALTER TABLE public.detalle_garantia_compra
  ADD COLUMN destino text NOT NULL DEFAULT 'venta'
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_payload->'items') LOOP
    ...
    -- S6-C: repartir el reclamo por destino (cajón mayoritario primero)
    v_restante := v_cant;
    FOR v_dc IN
  ...
  -- S6: conectar estado devolucion_garantia
...
begin
  ...
  loop
    -- 1) devolver lo reclamado al cajón correcto (S6-C)
    ...
    -- 2) retirar la reposición recibida (siempre entró al cajón de venta)
  ...
  -- S6: revertir estado de la compra si no quedan garantías activas
  if not exists (
       select 1 from garantias_compra
        where compra_id = v_gar.compra_id and id <> p_garantia_id and estado <> 'anulada'
     )
