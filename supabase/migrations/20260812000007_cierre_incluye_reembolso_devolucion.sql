-- El cierre cuenta el REEMBOLSO de una devolución de cliente como egreso del día,
-- por sede, con su método y cuenta. Se agrega el término en los 6 puntos donde ya
-- se cuenta el reembolso de garantía: total, por_sede, por_sede_metodo, por_cuenta,
-- egresos_detalle y arqueo (efectivo esperado). Verificado: la salida queda
-- IDÉNTICA para periodos sin reembolsos de devolución (no rompe el cálculo previo).
...
begin
  ...
  + coalesce((select sum(g.monto_devuelto) from garantias_venta g
       left join ventas gv on gv.id=g.venta_id
       left join ordenes_servicio go on go.id=g.orden_servicio_id
       where g.resolucion='devolver_dinero' and g.estado <> 'anulada'
         and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
         and (p_sede is null or coalesce(gv.sede_id, go.sede_id)=p_sede)),0)
  + coalesce((select sum(d.monto_reembolso) from devoluciones d
       where coalesce(d.monto_reembolso,0) > 0 and d.estado <> 'anulada'
         and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
         and (p_sede is null or d.sede_id=p_sede)),0)
  into v_egresos;
  ...
  from (select se.id, se.nombre,
      ...
      + coalesce((select sum(pc.monto) from pagos_cuenta pc join compras c on c.id=pc.compra_id where c.sede_destino_id=se.id and pc.tipo='pago' and coalesce(pc.anulado,false)=false
                  and (pc.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0)
      + coalesce((select sum(g.monto_devuelto) from garantias_venta g
                  left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
                  where coalesce(gv.sede_id, go.sede_id)=se.id and g.resolucion='devolver_dinero' and g.estado<>'anulada'
                  and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0)
      + coalesce((select sum(d.monto_reembolso) from devoluciones d
                  where d.sede_id=se.id and coalesce(d.monto_reembolso,0) > 0 and d.estado<>'anulada'
                  and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0) as egresos
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre, m.metodo,
    ...
    from sedes se join (
        ...
        union all select coalesce(gv.sede_id, go.sede_id), 'efectivo', 0::numeric, g.monto_devuelto
          from garantias_venta g left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
         where g.resolucion='devolver_dinero' and g.estado<>'anulada' and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all select d.sede_id, lower(d.metodo_reembolso), 0::numeric, d.monto_reembolso from devoluciones d
         where coalesce(d.monto_reembolso,0) > 0 and d.estado<>'anulada' and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre, m.cuenta,
    ...
    from sedes se join (
        ...
        union all select v.sede_id, nullif(trim(pc.cuenta_bancaria),''), pc.monto, 0::numeric from pagos_cuenta pc join ventas v on v.id=pc.venta_id
         where pc.tipo='cobro' and coalesce(pc.anulado,false)=false and (pc.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all select o.sede_id, nullif(trim(a.cuenta_bancaria),''), a.monto, 0::numeric from abonos a join ordenes_servicio o on o.id=a.orden_id
         where o.estado<>'cancelada' and (a.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all select co.sede_id, nullif(trim(ac.cuenta_bancaria),''), ac.monto, 0::numeric from abonos_cotizacion ac join cotizaciones co on co.id=ac.cotizacion_id
         where (ac.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        ...
        union all select c.sede_destino_id, nullif(trim(pc.cuenta_bancaria),''), 0::numeric, pc.monto from pagos_cuenta pc join compras c on c.id=pc.compra_id
         where pc.tipo='pago' and coalesce(pc.anulado,false)=false and (pc.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all select coalesce(gv.sede_id, go.sede_id), null::text, 0::numeric, g.monto_devuelto
          from garantias_venta g left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
         where g.resolucion='devolver_dinero' and g.estado<>'anulada' and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all select d.sede_id, nullif(trim(d.cuenta_reembolso),''), 0::numeric, d.monto_reembolso from devoluciones d
         where coalesce(d.monto_reembolso,0) > 0 and d.estado<>'anulada' and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
  ...
  from (
    ...
    union all
    select se.nombre, (g.fecha at time zone 'America/Bogota')::date,
      jsonb_build_object('sede_id', coalesce(gv.sede_id, go.sede_id), 'sede_nombre', se.nombre,
        'proveedor', coalesce(gv.cliente_nombre, go.cliente_nombre), 'concepto', 'Reembolso garantía venta #'||g.numero,
        'es_caja_menor', false, 'factura', null,
        'metodo', 'efectivo', 'cuenta', null,
        'total', g.monto_devuelto, 'fecha', (g.fecha at time zone 'America/Bogota')::date)
    from garantias_venta g left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
      join sedes se on se.id=coalesce(gv.sede_id, go.sede_id)
    where g.resolucion='devolver_dinero' and g.estado<>'anulada'
      and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
      and (p_sede is null or coalesce(gv.sede_id, go.sede_id)=p_sede)
    union all
    select se.nombre, (d.fecha at time zone 'America/Bogota')::date,
      jsonb_build_object('sede_id', d.sede_id, 'sede_nombre', se.nombre,
        'proveedor', v.cliente_nombre, 'concepto', 'Reembolso devolución #'||d.numero,
        'es_caja_menor', false, 'factura', null,
        'metodo', lower(d.metodo_reembolso), 'cuenta', nullif(trim(d.cuenta_reembolso),''),
        'total', d.monto_reembolso, 'fecha', (d.fecha at time zone 'America/Bogota')::date)
    from devoluciones d join sedes se on se.id=d.sede_id left join ventas v on v.id=d.venta_id
    where coalesce(d.monto_reembolso,0) > 0 and d.estado<>'anulada'
      and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre,
     ...
     - coalesce((select sum(case when lower(pc.metodo_pago)='efectivo' then pc.monto else 0 end) from pagos_cuenta pc join compras c on c.id=pc.compra_id
                where c.sede_destino_id=se.id and pc.tipo='pago' and coalesce(pc.anulado,false)=false and (pc.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0)
     - coalesce((select sum(g.monto_devuelto) from garantias_venta g
                left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
                where coalesce(gv.sede_id, go.sede_id)=se.id and g.resolucion='devolver_dinero' and g.estado<>'anulada'
                and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0)
     - coalesce((select sum(d.monto_reembolso) from devoluciones d
                where d.sede_id=se.id and lower(d.metodo_reembolso)='efectivo' and coalesce(d.monto_reembolso,0) > 0 and d.estado<>'anulada'
                and (d.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0) as esperado
