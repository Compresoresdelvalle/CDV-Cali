-- QA Campaña — Fase 11 — hallazgo F11-03 (continuación)
-- Misma corrección en el RPC de creación: `cotizaciones.cliente_nombre` es
-- NOT NULL, pero `fn_registrar_cotizacion` insertaba `p_cliente_nombre` sin
-- validar. Se exige nombre no vacío y se guarda con TRIM.
...
CREATE OR REPLACE FUNCTION public.fn_registrar_cotizacion(
  ...
  p_vigencia_dias integer DEFAULT NULL::integer,
  p_iva_pct numeric DEFAULT NULL::numeric,
  p_observaciones text DEFAULT NULL::text,
  p_condiciones_pago text DEFAULT NULL::text,
  p_tiempo_entrega_nota text DEFAULT NULL::text,
