-- ============================================================================
-- Cierres — Vista Avanzada (fix auditoría): por_cuenta omitía los abonos OT.
-- Los abonos no tienen cuenta_bancaria, así que la vista "por cuenta" no los
-- contaba y su suma de ingresos no cuadraba con ingresos_total. Se añaden con
-- cuenta NULL (la UI los etiqueta "Sin cuenta / efectivo"). Tras el fix,
-- sum(por_cuenta.ingresos) = ingresos_total.
-- Solo cambia el bloque por_cuenta; el resto de _fn_cierre_totales es idéntico.
-- ============================================================================
...
begin
  ...
  -- por_cuenta (FIX): incluye abonos OT con cuenta NULL ("Sin cuenta / efectivo")
  -- para que sum(ingresos) cuadre con ingresos_total.
  select coalesce(jsonb_agg(jsonb_build_object(
           'sede_id', x.sede_id, 'sede_nombre', x.sede_nombre,
    ...
    join (
        select v.sede_id, nullif(trim(v.cuenta_bancaria),'') as cuenta, v.total as ingresos, 0::numeric as egresos
         ...
         where v.anulada=false and v.origen in ('directa','ot')
           and (v.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all
        select o.sede_id, null::text as cuenta, a.monto as ingresos, 0::numeric as egresos
          from abonos a join ordenes_servicio o on o.id=a.orden_id
         where a.venta_id is null and o.estado<>'cancelada'
           and (a.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        union all
