-- Fix: en fn_dashboard_kpis, el bloque 'alertas' ordenaba con
-- `order by i.estado_stock desc` y `order by a->>'severity' desc`. Como
-- 'estado_stock'/'severity' son texto, el orden alfabético descendente pone
-- 'Bajo'/'warning' ANTES que 'Agotado'/'danger' (B > A, w > d) — justo al
-- revés de lo que se pretendía ("primero los agotados"). Con más de 10
-- productos entre Bajo y Agotado combinados, el `limit 10` interno incluso
-- podía dejar fuera productos realmente agotados a favor de productos con
-- stock simplemente bajo. Fix: ordenar por una condición booleana explícita
-- (Agotado primero), no por el texto en sí.
