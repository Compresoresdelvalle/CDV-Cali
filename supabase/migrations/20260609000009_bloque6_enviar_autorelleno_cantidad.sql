-- Traspasos · "Confirmar envío" falla con "Todos los items deben tener cantidad
-- enviada (> 0)" cuando un item quedó pickeado con cantidad_enviada = 0.
--
-- Causa raíz: la columna detalle_traspaso.cantidad_enviada tiene DEFAULT 0 y el
-- picking podía persistir ese 0 (en el front, `0 ?? solicitada` deja 0). Al
-- enviar, la validación > 0 lo rechaza y el traspaso queda atascado.
--
-- Defensa de servidor (para TODAS las sedes y traspasos ya atascados): al
-- enviar, los ítems marcados como pickeados pero con cantidad enviada 0/NULL
-- envían por defecto lo solicitado. La validación > 0 sólo aplica tras el
-- auto-relleno, así que únicamente bloquea ítems realmente sin pickear.
...
begin
  ...
  if p_accion = 'iniciar_picking' then
    ...
    -- Auto-relleno defensivo: ítem pickeado sin cantidad enviada => envía lo solicitado.
    update detalle_traspaso
       set cantidad_enviada = cantidad_solicitada
     where traspaso_id = p_traspaso_id
       and picking_completado = true
       and coalesce(cantidad_enviada, 0) <= 0
