-- Garantía de venta: el producto defectuoso que devuelve el cliente NO debe
-- volver como "segunda mano" vendible (si lo devolvieron es porque no funciona).
-- Ahora reingresa como CHATARRA: producto CHAT-<ref>, tipo='chatarra',
-- vendible=false (Ventas/Cotizaciones filtran vendible=true, así que no se puede
-- vender), precio/costo 0, solo para registro/control.
--
-- Requiere el valor 'chatarra' en el enum tipo_producto (migración previa).
-- Solo cambia el bloque de reingreso de items_devueltos dentro de
-- fn_abrir_garantia_venta; el resto de la función queda igual.
