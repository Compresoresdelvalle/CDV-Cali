-- S4-19: trg_compra_sumar_stock — lock FOR UPDATE de la fila productos antes de leer stock global
-- y recalcular costo_promedio, para serializar compras concurrentes del mismo producto.
...
BEGIN
  IF NEW.recibida = true AND (OLD.recibida = false OR OLD.recibida IS NULL) THEN
    ...
    FOR v_det IN SELECT * FROM detalle_compra WHERE compra_id = NEW.id LOOP
      ...
      -- y recalcular costo_promedio (promedio ponderado)
