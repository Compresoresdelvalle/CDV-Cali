-- Campaña revisión de problemas — motor del CIERRE de caja.
-- PROBLEMA: garantías de venta con resolucion='devolver_dinero' sacan efectivo de la
-- caja (garantias_venta.monto_devuelto) pero el motor del cierre NO las leía. El dinero
-- salía y el arqueo no lo veía.
--
-- FIX (hacia adelante): _fn_cierre_totales ahora cuenta los reembolsos de garantía de
-- venta del periodo como EGRESO, en TODOS los desgloses (total, por_sede, por_sede_metodo,
-- por_cuenta, egresos_detalle) y los RESTA del efectivo esperado del arqueo.
--
-- SUPUESTO EXPLÍCITO: el modelo NO guarda el método de pago del reembolso. Se asume
-- EFECTIVO (metodo='efectivo', cuenta=null) — lo más conservador para el arqueo físico:
-- reduce el efectivo esperado en caja, que es el escenario real de una devolución en mostrador.
--
-- Fecha que marca cuándo salió el dinero: garantias_venta.fecha (fecha_cierre está NULL en
-- todo el histórico devolver_dinero). Sede: la de la venta (o la OT) asociada.
-- Sólo devolver_dinero y estado <> 'anulada'.
...
begin
  ...
  -- + reembolsos de garantía de venta (devolver_dinero) del periodo
  ...
  from (select se.id, se.nombre,
      ...
      + coalesce((select sum(g.monto_devuelto) from garantias_venta g
                  ...
                  and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0) as egresos
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre, m.metodo,
    ...
    from sedes se join (
        ...
        -- reembolsos garantía venta -> egreso, método asumido EFECTIVO
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre, m.cuenta,
    ...
    from sedes se join (
        ...
        union all select c.sede_destino_id, nullif(trim(pc.cuenta_bancaria),''), 0::numeric, pc.monto from pagos_cuenta pc join compras c on c.id=pc.compra_id
         where pc.tipo='pago' and coalesce(pc.anulado,false)=false and (pc.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta
        -- reembolsos garantía venta -> egreso, sin cuenta bancaria (efectivo)
  ...
  from (
    ...
    union all
    -- reembolsos de garantía de venta como egreso en el detalle
  ...
  from (select se.id as sede_id, se.nombre as sede_nombre,
     ...
     -- reembolsos de garantía de venta (asumidos EFECTIVO) salen de la caja física
     - coalesce((select sum(g.monto_devuelto) from garantias_venta g
                left join ventas gv on gv.id=g.venta_id left join ordenes_servicio go on go.id=g.orden_servicio_id
                where coalesce(gv.sede_id, go.sede_id)=se.id and g.resolucion='devolver_dinero' and g.estado<>'anulada'
                and (g.fecha at time zone 'America/Bogota')::date between p_desde and p_hasta),0) as esperado
