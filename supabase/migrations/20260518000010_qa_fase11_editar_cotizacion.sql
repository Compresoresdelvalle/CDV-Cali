-- QA Campaña — Fase 11 (Ajustes Cotizaciones)
--
-- Cierra 3 hallazgos de la edición de cotizaciones, todos en
-- `CotizacionEditar.guardarCambios`, que escribía directo a las tablas:
--   * F4-08: UPDATE cotizaciones + DELETE/INSERT detalle NO transaccional →
--     si el INSERT falla, la cotización queda sin ítems.
--   * F11-01: `precio_unitario` y `subtotal`/`total` venían del cliente sin
--     recalcular en el servidor (misma clase de vulnerabilidad que F4-05 en
--     ventas) → se podía editar una cotización a $0.
--   * F11-02: se podía editar una cotización en estado `aprobada` (o ya
--     convertida en venta) vía URL directa, sin validación server-side.
--
-- Solución: RPC server-authoritative (mismo patrón que `fn_registrar_cotizacion`).
-- Recalcula precios desde `productos.precio_venta`; valida estado editable y
-- sede; hace cotización + detalle + cuentas en una sola transacción.
