-- S12 Bloque B: fn_dashboard_kpis
--  1) Ventas = ingreso real: origen in ('directa','ot') (antes solo 'directa', subdeclaraba ~9,7%)
--  2) Nuevos: ventas_semana_prev / ventas_mes_prev (delta correcto por periodo)
--  3) Nuevo: devoluciones_mes (devoluciones procesadas, valorizadas a precio de venta)
-- No se elimina ningun campo existente del contrato.
...
begin
  ...
  select jsonb_build_object(
    ...
    'ticket_promedio_mes', (select coalesce(avg(total),0)::integer from ventas where (fecha at time zone 'America/Bogota')::date >= v_mes_inicio and anulada = false and origen in ('directa','ot') and (v_rol = 'Admin' or sede_id = v_sede)),
    'devoluciones_mes', (
